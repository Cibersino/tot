...
| `tempo de leitura`                        | Nome localizado da extensão do navegador em `pt-br`, e também expressão comum para duração de leitura quando o contexto permitir. | Não confundir com `teste de velocidade de leitura` nem com `tempo estimado de leitura`.                                                                                   |
...
| `testes incorporados`                     | Arquivos iniciais incluídos com a app para o teste.                                                                               | Evitar `testes embutidos` como padrão se a superfície for de usuário.                                                                                                     |
...
| `autorização`, `autorizar`, `autorizado`                          | grafias ou formas não brasileiras                              |

## 7. Revisão

O estado inicial recomendado para `pt-br` é `needs native review` até revisão humana nativa ou equivalente. A proximidade com o espanhol não basta para fechar qualidade idiomática.

Riscos principais na revisão de `pt-br`:

* calcos do espanhol em estrutura de confirmação, ordem de palavras e vocabulário técnico;
* calcos do inglês em compostos e termos de produto;
* neutralização excessiva em direção a um português genérico;
* infiltração de formas europeias como `ficheiro`, `utilizador`, `ecrã` ou `guardar` para save;
* uso indevido de `fila` quando a superfície quer `linha`;
* alternância desnecessária entre forma brasileira corrente e alternativa formal.

Novas decisões devem ser adicionadas a esta guia apenas quando afetarem consistência futura de `pt-br`. Dúvidas gerais de app, tokens protegidos, placeholders, fallback, contrato runtime ou composição técnica pertencem à guia global ou à política i18n correspondente.
