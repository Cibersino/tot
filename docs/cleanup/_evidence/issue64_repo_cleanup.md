...
**0.1 Reading map**
- Block order: header + `'use strict'` → `const { contextBridge, ipcRenderer } = require('electron');` → `const api = { ... }` (grouped by section comments) → `contextBridge.exposeInMainWorld('electronAPI', api);`.
- Linear reading breaks (identifier + micro-quote):
  - Single large `api` literal bundles multiple responsibilities; jumps are separated mainly by comment dividers (e.g., `Central Crono API`, `APIs for the floating window`, `Startup handshake`).

**0.2 Preload surface contract map (contextBridge)**
- Exposes: `contextBridge.exposeInMainWorld('electronAPI', api)`; no other globals.
- Exposed keys on `electronAPI` (grouped by section in-file):
  - Clipboard/editor/presets/settings: `readClipboard`, `openEditor`, `checkForUpdates`, `openPresetModal`, `openDefaultPresetsFolder`, `getCurrentText`, `setCurrentText`, `getAppConfig`, `getAppVersion`, `getAppRuntimeInfo`, `openExternalUrl`, `openAppDoc`, `onCurrentTextUpdated`, `getSettings`, `onPresetCreated`, `getDefaultPresets`, `setSelectedPreset`, `requestDeletePreset`, `requestRestoreDefaults`, `notifyNoSelectionEdit`, `forceClearEditor`.
  - Menu: `onMenuClick` (returns unsubscribe).
  - Mode/settings: `setModeConteo`, `onSettingsChanged` (returns unsubscribe).
  - Crono: `sendCronoToggle`, `sendCronoReset`, `setCronoElapsed`, `getCronoState`, `onCronoState` (returns unsubscribe).
  - Flotante: `openFlotanteWindow`, `closeFlotanteWindow`, `onFlotanteClosed` (returns unsubscribe).
  - Editor: `onEditorReady` (returns unsubscribe).
  - Startup handshake: `sendStartupRendererCoreReady`, `onStartupReady` (returns unsubscribe), `sendStartupSplashRemoved`.
- Listener semantics (as implemented here):
  - Some listeners do NOT return unsubscribe (e.g., `onCurrentTextUpdated`, `onPresetCreated`).
  - Others return unsubscribe via `ipcRenderer.removeListener(<channel>, <fn>)` (e.g., `onMenuClick`, `onSettingsChanged`, `onCronoState`, `onFlotanteClosed`, `onEditorReady`, `onStartupReady`).

**0.3 IPC contract (mechanical; all occurrences)**
- `ipcRenderer.invoke` channels:
  - `'clipboard-read-text'`, `'open-editor'`, `'check-for-updates'` (`{ manual }`), `'open-preset-modal'` (`payload`), `'open-default-presets-folder'`,
    `'get-current-text'`, `'set-current-text'` (`text`),
    `'get-app-config'`, `'get-app-version'`, `'get-app-runtime-info'`,
    `'open-external-url'` (`url`), `'open-app-doc'` (`docKey`),
    `'get-settings'`, `'get-default-presets'`,
    `'set-selected-preset'` (`name`), `'request-delete-preset'` (`name`),
    `'request-restore-defaults'`, `'notify-no-selection-edit'`, `'force-clear-editor'`,
    `'set-mode-conteo'` (`mode`),
    `'crono-get-state'`,
    `'flotante-open'`, `'flotante-close'`.
- `ipcRenderer.send` channels:
  - `'crono-toggle'`, `'crono-reset'`, `'crono-set-elapsed'` (`ms`),
    `'startup:renderer-core-ready'`, `'startup:splash-removed'`.
- `ipcRenderer.on` channels (forwarded payload to cb as implemented):
  - `'current-text-updated'` → `text`
  - `'preset-created'` → `preset`
  - `'menu-click'` → `payload`
  - `'settings-updated'` → `newSettings`
  - `'crono-state'` → `state`
  - `'flotante-closed'` → (no payload)
  - `'editor-ready'` → (no payload)
  - `'startup:ready'` → (no payload)
- `ipcRenderer.removeListener` channels (unsubscribe paths):
  - `'menu-click'`, `'settings-updated'`, `'crono-state'`, `'flotante-closed'`, `'editor-ready'`, `'startup:ready'`.
- No `ipcMain.*` or `webContents.send` in this file.

**0.4 Invariants / fallbacks (anchored)**
- `checkForUpdates` defaults `manual = false`.
- `openPresetModal` accepts optional payload (per in-file comment).
- Some listener wrappers isolate callback errors with `try/catch` (e.g., `onMenuClick`); others call `cb(...)` directly (e.g., `onCurrentTextUpdated`, `onPresetCreated`).
- Unsubscribe functions use best-effort `removeListener` under `try/catch`.
