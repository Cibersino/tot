...
| electron/current_text_snapshots_main.js | CHANGED | [### electron/current_text_snapshots_main.js](#electroncurrent_text_snapshots_mainjs) | OK | - `registerIpc(ipcMain, ...)` validaba solo existencia de `ipcMain` y no su contrato requerido (`ipcMain.handle`), dejando fallo implícito si estaba mal cableado.<br>- El resto de handlers IPC del archivo ya devolvía resultados explícitos (`{ ok:false, code }`) con diagnósticos no silenciosos en rutas de fallo.<br>- No se detectó drift de surface/ordering en channels `current-text-snapshot-*`. | - Se endureció el fail-fast requerido en `registerIpc` verificando explícitamente `ipcMain.handle`.<br>- Se preservaron sin cambios channels, payload/return shapes, side effects y orden de registro IPC.<br>- No se añadieron wrappers ni refactors fuera del scope Level 3. | - node --check electron/current_text_snapshots_main.js OK.<br>- Grep/manual confirma inventario completo de `ipcMain.handle('current-text-snapshot-save'/'current-text-snapshot-select'/'current-text-snapshot-load')` y paths IPC-adjacent de owner/`textState`.<br>- Healthy-path smoke: sin cambios en channels/payloads/ordering/side-effects. | N - ajuste local de failure-path/contrato de registro en main; sin cambios de contrato healthy-path ni cambios cross-file. |
| electron/editor_find_main.js | NO CHANGE | [### electron/editor_find_main.js](#electroneditor_find_mainjs) | OK | - No se detectó coexistencia no clasificada: contrato requerido de `registerIpc` ya está en fail-fast explícito y handlers IPC están guardados por autorización de sender.<br>- Las rutas send/drop (`safeSendToFindWindow`, `stopFindInPage`) ya son best-effort no bloqueantes con diagnóstico deduplicado cuando falla la acción intentada.<br>- No hay drift de channels/payloads/ordering en `editor-find-*`. | - none | - node --check electron/editor_find_main.js OK.<br>- Grep/manual confirma inventario exhaustivo de `ipcMain.handle('editor-find-*')` y sends `editor-find-init/state/focus-query`.<br>- Healthy-path smoke: sin cambios en channels/payloads/ordering/side-effects. | N - archivo ya alineado en Level 3; no requiere cambios locales ni cross-file. |
| electron/updater.js | CHANGED | [### electron/updater.js](#electronupdaterjs) | OK | - `registerIpc(ipcMain, ...)` permitía missing/invalid registrar en modo silencioso (saltaba el handle sin diagnóstico/fail-fast).<br>- El resto del path bridge (`ipcMain.handle('check-for-updates')`) ya retornaba shape explícito `{ ok, error? }` y mantenía fallbacks no-silenciosos en runtime.<br>- No se detectó drift de surface en el canal `check-for-updates`. | - Se endureció el fail-fast requerido en `registerIpc` con validación explícita de `ipcMain.handle`.<br>- Se mantuvo sin cambios el handler `check-for-updates` (channel/payload/return shape) y el flujo healthy-path de `checkForUpdates`.<br>- No se introdujeron wrappers ni refactors fuera del scope Level 3. | - node --check electron/updater.js OK.<br>- Grep/manual confirma inventario IPC-adjacent completo (`registerIpc`, `ipcMain.handle('check-for-updates')`, callbacks `mainWinRef/currentLanguageRef`).<br>- Healthy-path smoke: sin cambios en channels/payloads/ordering/side-effects. | N - ajuste local de failure-path/contrato de registro en main; sin cambios de contrato healthy-path ni cambios cross-file. |
| electron/link_openers.js | CHANGED | [### electron/link_openers.js](#electronlink_openersjs) | OK | - `registerLinkIpc({ ipcMain, ... })` carecía de validación explícita del contrato requerido `ipcMain.handle` (fallo implícito por TypeError).<br>- Los handlers `open-external-url` y `open-app-doc` ya devolvían rutas de degradación explícitas (`{ ok:false, reason }`) sin fallback silencioso.<br>- No se detectó drift de channels/payloads/ordering en `open-external-url` y `open-app-doc`. | - Se agregó fail-fast explícito para contrato requerido `ipcMain.handle` en `registerLinkIpc`.<br>- Se preservaron sin cambios channels, payload/return shapes y side effects de apertura (`openExternal`/`openPath`).<br>- No se añadieron wrappers ni refactors fuera del scope Level 3. | - node --check electron/link_openers.js OK.<br>- Grep/manual confirma inventario bridge completo (`registerLinkIpc`, `ipcMain.handle('open-external-url'/'open-app-doc')`, `shell.openExternal`, `shell.openPath`).<br>- Healthy-path smoke: sin cambios en channels/payloads/ordering/side-effects. | N - ajuste local de failure-path/contrato de registro en main; sin cambios de contrato healthy-path ni cambios cross-file. |
| electron/menu_builder.js | NO CHANGE | [### electron/menu_builder.js](#electronmenu_builderjs) | OK | - Todos los paths bridge/IPC-adjacent ya están clasificados y manejados sin fallback silencioso (`menu-click` send, hooks opcionales, acciones dev sobre `webContents`).<br>- Los drops esperables por ausencia/cierre de ventana usan `warnOnce` con keys estables/acotadas y no rompen flujo.<br>- No hay drift de channels/payloads/ordering en `menu-click` ni en el wiring del menú. | - none | - node --check electron/menu_builder.js OK.<br>- Grep/manual confirma inventario completo de paths bridge/IPC-adjacent (`mainWindow.webContents.send('menu-click')`, `onOpenLanguage`, `BrowserWindow.getFocusedWindow`, `webContents.reload/reloadIgnoringCache/toggleDevTools`).<br>- Healthy-path smoke: sin cambios en channels/payloads/ordering/side-effects. | N - archivo ya alineado en Level 3; no requiere cambios locales ni cross-file. |
...
| ID | Location anchor | Bridge member/path | Class | Current handling | Required handling | Action taken | Dedupe key (if any) |
|----|------------------|--------------------|-------|------------------|-------------------|--------------|---------------------|
| CTSM01 | IPC registration contract (`L252`-`L255`) startup/rare | `registerIpc(ipcMain, ...)` dependency on `ipcMain.handle` | required startup dependency | Before: checked only truthy `ipcMain`; invalid registrar contract failed later implicitly. | Fail fast with explicit diagnostic when required registrar contract is invalid. | Updated guard to `if (!ipcMain || typeof ipcMain.handle !== 'function') throw ...`. | n/a |
| CTSM02 | IPC options resolver (`L252`, `L257`-`L263`) startup/interactive | `getWindows()` callback dependency used by `resolveMainWin` | optional capability | Guarded by `typeof getWindows === 'function'`; fallback to `null` owner window. | Guard + continue with degraded owner-window resolution. | No change; existing guard already aligns with optional capability handling. | n/a |
| CTSM03 | owner-window resolution (`L172`-`L181`) interactive | `BrowserWindow.fromWebContents(event.sender)` in `resolveOwnerWin` | optional capability | Guarded with try/catch fallback to main window resolver; no crash on invalid sender context. | Guard + continue with safe fallback + diagnostic only when needed. | No change; already compliant optional capability pattern. | n/a |
| CTSM04 | IPC handler (`L265`) interactive | `ipcMain.handle('current-text-snapshot-save', ...)` | optional capability | Handler uses guarded root/path checks and explicit `{ ok:false, code }` returns; failures logged via catch. | Guard + continue with explicit degraded result + diagnostics. | No change; already compliant. | n/a |
| CTSM05 | IPC-adjacent text path (`L302`) interactive | `textState.getCurrentText()` used by snapshot-save flow | optional capability | Indirectly guarded by surrounding try/catch; failures return `{ ok:false, code:'WRITE_FAILED' }`. | Guard + continue with explicit degraded result + diagnostics. | No change; current try/catch handling remains explicit and non-silent. | n/a |
| CTSM06 | IPC handler (`L328`) interactive | `ipcMain.handle('current-text-snapshot-select', ...)` | optional capability | Handler uses guarded root/path validation and explicit `{ ok:false, code }` returns; failures logged via catch. | Guard + continue with explicit degraded result + diagnostics. | No change; already compliant. | n/a |
| CTSM07 | IPC handler (`L358`) interactive | `ipcMain.handle('current-text-snapshot-load', ...)` | optional capability | Handler validates rel-path/open dialog/confirm and returns explicit failure codes on invalid paths/json/schema. | Guard + continue with explicit degraded result + diagnostics. | No change; already compliant. | n/a |
| CTSM08 | IPC-adjacent apply path (`L409`-`L412`) interactive | `textState.applyCurrentText(...)` used by snapshot-load flow | optional capability | Indirectly guarded by surrounding try/catch; failures return `{ ok:false, code:'READ_FAILED' }`. | Guard + continue with explicit degraded result + diagnostics. | No change; current try/catch handling remains explicit and non-silent. | n/a |

Level 4 evidence:
- N. No Level 4 escalation applies: update is local failure-path alignment in `electron/current_text_snapshots_main.js`; IPC surface, channel names, payload/return shapes, side effects, and healthy-path ordering/timing were not changed.

Healthy-path confirmation: the healthy-path contract/timing was preserved (no changes to channels, payload/return shapes, side effects, or execution ordering when bridge wiring is valid).

Failure-path confirmation: failure-mode handling is aligned to the convention (required IPC registrar contract now fails fast explicitly; optional handlers/IPC-adjacent paths remain guarded with explicit degraded returns).

### electron/editor_find_main.js

| ID | Location anchor | Bridge member/path | Class | Current handling | Required handling | Action taken | Dedupe key (if any) |
|----|------------------|--------------------|-------|------------------|-------------------|--------------|---------------------|
| EFM01 | IPC registration contract (`L717`-`L720`) startup/rare | `registerIpc(ipcMain)` dependency on `ipcMain.handle` | required startup dependency | Explicit fail-fast (`throw`) when `ipcMain.handle` is unavailable. | Fail fast with clear diagnostic for invalid init contract. | No change; already compliant. | n/a |
| EFM02 | IPC auth wrapper (`L707`-`L715`) interactive | `ipcMain.handle(channel, ...)` via `registerAuthorizedFindIpc` | optional capability | Sender authorization guard returns `{ ok:false, error:'unauthorized' }` + deduped warning. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | per-channel (`editorFind.ipc.*.unauthorized`) |
| EFM03 | IPC channel registration (`L722`-`L728`) interactive/high-frequency | `ipcMain.handle('editor-find-set-query', ...)` | optional capability | Authorized path delegates to `setQuery`; unauthorized path guarded and deduped. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | `editorFind.ipc.setQuery.unauthorized` |
| EFM04 | IPC channel registration (`L730`-`L736`) interactive | `ipcMain.handle('editor-find-next', ...)` | optional capability | Authorized path delegates to `navigate(true)`; unauthorized path guarded and deduped. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | `editorFind.ipc.next.unauthorized` |
| EFM05 | IPC channel registration (`L738`-`L744`) interactive | `ipcMain.handle('editor-find-prev', ...)` | optional capability | Authorized path delegates to `navigate(false)`; unauthorized path guarded and deduped. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | `editorFind.ipc.prev.unauthorized` |
| EFM06 | IPC channel registration (`L746`-`L752`) interactive | `ipcMain.handle('editor-find-close', ...)` | optional capability | Authorized path delegates to close flow; unauthorized path guarded and deduped. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | `editorFind.ipc.close.unauthorized` |
| EFM07 | Authorization path (`L701`-`L705`) interactive | `event.sender === findWin.webContents` in `isAuthorizedFindSender` | optional capability | Strict sender ownership check; unauthorized senders are rejected by wrapper without crash. | Guard + continue with explicit unauthorized response. | No change; already compliant. | n/a |
| EFM08 | send side action (`L103`-`L109`) interactive/high-frequency | `safeSendToFindWindow('editor-find-init'/'editor-find-state', ...)` -> `webContents.send(...)` | best-effort side action | Drop when window is unavailable/loading; on send exception logs deduped "failed (ignored)" warning. | Attempt send; drop without breaking flow; dedupe on repeatable send failures. | No change; already compliant. | `editorFind.send.editor-find-init`; `editorFind.send.editor-find-state` |
| EFM09 | send side action (`L252`-`L254`) interactive | `safeSendToFindWindow('editor-find-focus-query', ...)` -> `webContents.send(...)` | best-effort side action | Same best-effort path as EFM08; dropped safely when unavailable/loading, deduped warning on send failure. | Attempt send; drop without breaking flow; dedupe on repeatable send failures. | No change; already compliant. | `editorFind.send.editor-find-focus-query` |
| EFM10 | editor search side action (`L136`-`L147`) interactive/high-frequency | `editorWin.webContents.stopFindInPage('clearSelection')` | best-effort side action | Wrapped in try/catch; failures are ignored with deduped warning and flow continues. | Drop without breaking flow + diagnostic when clear-selection fails. | No change; already compliant. | `editorFind.stopFind.clearSelection` |
| EFM11 | editor search action (`L154`-`L171`) interactive/high-frequency | `editorWin.webContents.findInPage(...)` | optional capability | Editor availability guard + explicit error return; runtime exception logged and surfaced as `{ ok:false, error }`. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | `editorFind.runFind.noEditor` |

Level 4 evidence:
- N. No Level 4 escalation applies: `electron/editor_find_main.js` is already aligned to Level 3 and requires no healthy-path or cross-file changes.

Healthy-path confirmation: the healthy-path contract/timing was preserved (no changes to channels, payload/return shapes, side effects, or execution ordering when bridge wiring is valid).

Failure-path confirmation: failure-mode handling is aligned to the convention (required registrar contract fail-fast is explicit; optional IPC capabilities are guarded; best-effort send/search side actions remain non-breaking with diagnostics).

### electron/updater.js

| ID | Location anchor | Bridge member/path | Class | Current handling | Required handling | Action taken | Dedupe key (if any) |
|----|------------------|--------------------|-------|------------------|-------------------|--------------|---------------------|
| UP01 | IPC registration contract (`L292`-`L315`) startup/rare | `registerIpc(ipcMain, ...)` dependency on `ipcMain.handle` | required startup dependency | Before: invalid `ipcMain` silently skipped IPC registration (`if (ipcMain && typeof ipcMain.handle === 'function')`). | Fail fast with explicit diagnostic for invalid required registrar contract. | Updated to explicit guard + `throw new Error('[updater] registerIpc requires ipcMain')`. | n/a |
| UP02 | IPC channel registration (`L304`-`L316`) interactive/rare | `ipcMain.handle('check-for-updates', ...)` | optional capability | Handler catches update-flow exceptions and returns stable `{ ok:false, error }`; normal path returns `{ ok:true }`. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | n/a |
| UP03 | IPC-adjacent callback dependency (`L292`-`L299`, `L311`) startup/interactive | `currentLanguageRef` callback used for update-check language resolution | optional capability | Guarded by `typeof langRef/currentLanguageRef === 'function'`; fallback to `DEFAULT_LANG`. | Guard + continue with safe fallback + diagnostic where required. | No change; existing guards already keep runtime non-breaking. | n/a |
| UP04 | IPC-adjacent callback dependency (`L292`-`L299`, `L160`, `L245`) startup/interactive | `mainWinRef` callback used to resolve dialog owner window | optional capability | Guarded by `typeof mainRef/mainWinRef === 'function'`; fallback `null` window and skip dialog when unavailable/destroyed. | Guard + continue with safe degraded behavior (no invalid dialog path). | No change; existing guards already compliant. | n/a |

Level 4 evidence:
- N. No Level 4 escalation applies: update is local failure-path alignment in `electron/updater.js`; IPC surface, channel names, payload/return shapes, side effects, and healthy-path ordering/timing were not changed.

Healthy-path confirmation: the healthy-path contract/timing was preserved (no changes to channels, payload/return shapes, side effects, or execution ordering when bridge wiring is valid).

Failure-path confirmation: failure-mode handling is aligned to the convention (required IPC registrar contract now fails fast explicitly; optional IPC/callback dependencies remain guarded with explicit degraded behavior).

### electron/link_openers.js

| ID | Location anchor | Bridge member/path | Class | Current handling | Required handling | Action taken | Dedupe key (if any) |
|----|------------------|--------------------|-------|------------------|-------------------|--------------|---------------------|
| LO01 | IPC registration contract (`L90`-`L95`) startup/rare | `registerLinkIpc({ ipcMain, app, shell })` dependency on `ipcMain.handle` | required startup dependency | Before: missing/invalid registrar failed implicitly on first `ipcMain.handle` call. | Fail fast with explicit diagnostic when required IPC registrar contract is invalid. | Added explicit guard + `throw new Error('[link_openers] registerLinkIpc requires ipcMain')`. | n/a |
| LO02 | IPC channel registration (`L95`-`L122`) interactive/rare | `ipcMain.handle('open-external-url', ...)` | optional capability | Guarded parsing/allowlist checks return `{ ok:false, reason:'blocked' }`; handler catch returns `{ ok:false, reason:'error' }`. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | n/a |
| LO03 | open side action (`L116`) interactive/rare | `shell.openExternal(parsed.toString())` | optional capability | Executed in guarded handler; runtime failures caught and returned as `{ ok:false, reason:'error' }`. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | n/a |
| LO04 | IPC channel registration (`L124`-`L198`) interactive/rare | `ipcMain.handle('open-app-doc', ...)` | optional capability | Input/dev/package/file guards return explicit reasons (`blocked/not_found/not_available_in_dev/open_failed/error`) with diagnostics. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | n/a |
| LO05 | open side action (`L78`-`L85`, call-sites `L151`,`L166`,`L183`,`L193`) interactive/rare | `shell.openPath(filePath)` via `openPathWithLog` | best-effort side action | Intended open action is attempted; failures are dropped with warning and `{ ok:false, reason:'open_failed' }`. | Attempt side action; on failure drop without breaking flow + diagnostic. | No change; already compliant. | n/a |
| LO06 | temp-path fallback (`L57`-`L68`) interactive/rare | `app.getPath('temp')` in `getTempDir` | optional capability | Guarded fallback to `os.tmpdir()` with deduped warning when `app.getPath('temp')` fails. | Guard + continue with degraded fallback + deduped diagnostic. | No change; already compliant. | `link_openers.tempPath.fallback` |
| LO07 | app doc resolution dependency (`L132`-`L188`) interactive/rare | `app.isPackaged` / `app.getAppPath()` | optional capability | Used under guarded branches and outer try/catch; failures return `{ ok:false, reason:'error' }`. | Guard + continue with explicit degraded result + diagnostic. | No change; already compliant. | n/a |

Level 4 evidence:
- N. No Level 4 escalation applies: update is local failure-path alignment in `electron/link_openers.js`; IPC surface, channel names, payload/return shapes, side effects, and healthy-path ordering/timing were not changed.

Healthy-path confirmation: the healthy-path contract/timing was preserved (no changes to channels, payload/return shapes, side effects, or execution ordering when bridge wiring is valid).

Failure-path confirmation: failure-mode handling is aligned to the convention (required IPC registrar contract now fails fast explicitly; optional capabilities remain guarded; best-effort open side actions remain non-breaking with diagnostics).

### electron/menu_builder.js

| ID | Location anchor | Bridge member/path | Class | Current handling | Required handling | Action taken | Dedupe key (if any) |
|----|------------------|--------------------|-------|------------------|-------------------|--------------|---------------------|
| MB01 | menu dispatch helper (`L261`-`L291`) interactive | `mainWindow.webContents.send('menu-click', payload)` | best-effort side action | Missing/destroyed window branches drop action with `warnOnce`; send exception is caught and logged as failed (ignored). | Attempt send; if action is dropped do not break flow + diagnostic. | No change; already compliant. | `menu_builder.sendMenuClick.noWindow`; `menu_builder.sendMenuClick.destroyed`; `menu_builder.sendMenuClick.sendFailed` |
| MB02 | menu target resolver (`L227`-`L231`, `L264`) startup/interactive | `opts.resolveMainWindow` / `opts.mainWindow` callback path | optional capability | Resolver is guarded with safe fallback to `null`; `sendMenuClick` handles null target non-fatally with diagnostics. | Guard + continue with capability disabled + diagnostic when action drops. | No change; already compliant. | covered by MB01 keys |
| MB03 | pre-ready gate (`L232`-`L245`) startup/interactive | `opts.isMenuEnabled` gate for dispatch (`canDispatchMenuAction`) | optional capability | Disabled state blocks dispatch and emits deduped warning; flow continues without crash. | Guard + continue with explicit degraded behavior + diagnostic. | No change; already compliant. | `menu_builder.inert:<actionId>` |
| MB04 | language hook dispatch (`L255`-`L356`) interactive/rare | `opts.onOpenLanguage` callback invocation | optional capability | Missing hook warns once and continues; callback failures caught with `errorOnce`. | Guard + continue with capability disabled + diagnostic. | No change; already compliant. | `menu_builder.onOpenLanguage.missing`; `menu_builder.onOpenLanguage` |
| MB05 | dev target resolver (`L247`-`L253`) interactive/rare | `BrowserWindow.getFocusedWindow()` + fallback `resolveMainWindow()` | optional capability | Guarded selection of target window; null target makes dev action a no-op without crash. | Guard + continue safely when target absent. | No change; already compliant. | n/a |
| MB06 | dev menu action (`L440`-`L448`) interactive/rare | `target.webContents.reload()` | best-effort side action | Executes only when target exists; failures are caught and deduped as ignored. | Attempt action; drop on failure without breaking flow + diagnostic. | No change; already compliant. | `menu_builder.devReload` |
| MB07 | dev menu action (`L458`-`L466`) interactive/rare | `target.webContents.reloadIgnoringCache()` | best-effort side action | Executes only when target exists; failures are caught and deduped as ignored. | Attempt action; drop on failure without breaking flow + diagnostic. | No change; already compliant. | `menu_builder.devForceReload` |
| MB08 | dev menu action (`L476`-`L484`) interactive/rare | `target.webContents.toggleDevTools()` | best-effort side action | Executes only when target exists; failures are caught and deduped as ignored. | Attempt action; drop on failure without breaking flow + diagnostic. | No change; already compliant. | `menu_builder.toggleDevTools` |

Level 4 evidence:
- N. No Level 4 escalation applies: `electron/menu_builder.js` is already aligned to Level 3 and requires no healthy-path or cross-file changes.

Healthy-path confirmation: the healthy-path contract/timing was preserved (no changes to channels, payload/return shapes, side effects, or execution ordering when bridge wiring is valid).

Failure-path confirmation: failure-mode handling is aligned to the convention (optional hooks/dispatch guards are explicit and non-silent; best-effort window actions remain non-breaking with stable diagnostics).
