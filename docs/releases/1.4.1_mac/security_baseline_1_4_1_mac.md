...
Registro de este release:
el bundle de macOS mantiene `electron/**`, `public/**`, `i18n/**`, `LICENSE`, `PRIVACY.md`, `public/third_party_licenses/*`, `electron/assets/ocr_google_drive/*` y el mismo set de dependencias runtime de `1.4.0`; el contenido desempaquetado esperado sigue acotado al runtime nativo `@img/sharp-darwin-arm64/**`. El delta frente a `1.4.0` no agrega ni remueve dependencias de produccion redistribuidas.
