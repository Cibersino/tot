// electron/text_extraction_platform/text_extraction_preconditions_ipc.js
'use strict';

// =============================================================================
// Overview
// =============================================================================
// Main-process IPC wrapper for text extraction start preconditions.
// Responsibilities:
...
const log = Log.get('text-extraction-preconditions');
...
function assertValidPreconditionContext(context) {
  if (!context || typeof context !== 'object') {
    throw new Error('text-extraction precondition context invalid');
  }
  if (!Array.isArray(context.openSecondaryWindows)) {
    throw new Error('text-extraction precondition context missing openSecondaryWindows array');
  }
  if (typeof context.stopwatchRunning !== 'boolean') {
    throw new Error('text-extraction precondition context missing stopwatchRunning boolean');
  }

  for (const item of context.openSecondaryWindows) {
    if (!item || typeof item !== 'object') {
      throw new Error('text-extraction precondition window entry invalid');
    }
    if (typeof item.id !== 'string' || !item.id.trim()) {
      throw new Error('text-extraction precondition window entry missing id');
    }
    if (typeof item.isOpen !== 'boolean') {
      throw new Error('text-extraction precondition window entry missing isOpen boolean');
    }
  }
}
...
function isAuthorizedSender(event, mainWin) {
  try {
    ...
    if (!mainWin || senderWin !== mainWin) {
      log.warnOnce(
        'text_extraction_preconditions.unauthorized',
        'text-extraction-check-preconditions unauthorized (ignored).'
      );
      return false;
    }
    return true;
  } catch (err) {
    log.warn('text-extraction-check-preconditions sender validation failed:', err);
    return false;
  }
}
...
function registerIpc(ipcMain, { getWindows, getPreconditionContext } = {}) {
  if (!ipcMain || typeof ipcMain.handle !== 'function') {
    throw new Error('[text_extraction_preconditions] registerIpc requires ipcMain');
  }
  if (typeof getWindows !== 'function') {
    throw new Error('[text_extraction_preconditions] registerIpc requires getWindows()');
  }
  if (typeof getPreconditionContext !== 'function') {
    throw new Error('[text_extraction_preconditions] registerIpc requires getPreconditionContext()');
  }
  ...
  ipcMain.handle('text-extraction-check-preconditions', async (event) => {
    try {
      ...
      if (openSecondaryWindows.length > 0 || stopwatchRunning) {
        ...
        log.warn('text-extraction precondition_rejected:', blocked.detailsSafeForLogs);
        return blocked;
      }

      return buildReadyResult();
    } catch (err) {
      log.error('text-extraction precondition check failed:', err);
      return {
        ok: false,
        canStart: false,
        state: 'failure',
...
// =============================================================================
// End of electron/text_extraction_platform/text_extraction_preconditions_ipc.js
// =============================================================================
